...
class View extends Simpla
{
	...
	public function __construct()
	{
		...
		else
		{
            ...
            $this->design->smarty->registerPlugin("function", "get_bestsellers_products",	array($this, 'get_bestsellers_products_plugin'));
    ...
    public function get_bestsellers_products_plugin($params, &$smarty)
    {
        if(!isset($params['visible']))
            $params['visible'] = 1;
            $params['bestsellers'] = 1;
        if(!empty($params['var']))
        {
            foreach($this->products->get_products($params) as $p)
                $products[$p->id] = $p;
