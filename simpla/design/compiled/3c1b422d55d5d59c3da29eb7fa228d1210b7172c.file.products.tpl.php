<?php /* Smarty version Smarty-3.1.18, created on 2022-04-20 14:56:55
         compiled from "simpla\design\html\products.tpl" */ ?>
<?php /*%%SmartyHeaderCode:545857222625950f4d49039-86261695%%*/if(!defined('SMARTY_DIR')) exit('no direct access allowed');
$_valid = $_smarty_tpl->decodeProperties(array (
  ...
  'nocache_hash' => '545857222625950f4d49039-86261695',
  'function' => 
  array (
    'category_select' => 
    array (
  ...
  'unifunc' => 'content_625950f4e1ad61_11861624',
  'variables' => 
  array (
    'manager' => 0,
    'category' => 0,
...
<?php if ($_valid && !is_callable('content_625950f4e1ad61_11861624')) {function content_625950f4e1ad61_11861624($_smarty_tpl) {?><?php if (!is_callable('smarty_modifier_truncate')) include 'C:\\xampp\\htdocs\\simpla\\www\\Smarty\\libs\\plugins\\modifier.truncate.php';
?>
...
?>
		<div class="<?php if (!$_smarty_tpl->tpl_vars['product']->value->visible) {?>invisible<?php }?> <?php if ($_smarty_tpl->tpl_vars['product']->value->featured) {?>featured<?php }?> <?php if ($_smarty_tpl->tpl_vars['product']->value->bestsellers) {?>bestsellers<?php }?>  row">
...
" target="_blank"></a>			
				...
				<a class="bestsellers"  title="Бестселлеры"           href="#"></a>
		...
		<div id="action">
			...
			<select name="action">
				...
				<option value="set_bestsellers">Сделать бестселлер</option>
				<option value="unset_bestsellers">Отменить бестселлер</option>
...
">Рекомендуемые</a></li>
		<li <?php if ($_smarty_tpl->tpl_vars['filter']->value=='bestsellers') {?>class="selected"<?php }?>><a href="<?php echo $_smarty_tpl->smarty->registered_plugins[Smarty::PLUGIN_FUNCTION]['url'][0][0]->url_modifier(array('keyword'=>null,'brand_id'=>null,'category_id'=>null,'page'=>null,'filter'=>'bestsellers'),$_smarty_tpl);?>
">Бестселлеры</a></li>
...
$(function() {
	...
	$("a.enable").click(function() {
		let icon        = $(this);
		let line        = icon.closest("div.row");
		let id          = line.find('input[type="checkbox"][name*="check"]').val();
		let state       = line.hasClass('invisible')?1:0;
...
'},
	...
	$("a.featured").click(function() {
		...
		let line        = icon.closest("div.row");
		let id          = line.find('input[type="checkbox"][name*="check"]').val();
		let state       = line.hasClass('featured')?0:1;
		icon.addClass('loading_icon');
		$.ajax({
			type: 'POST',
			url: 'ajax/update_object.php',
...
'},
	...
	$("a.bestsellers").click(function() {
		let icon        = $(this);
		let line        = icon.closest("div.row");
		let id          = line.find('input[type="checkbox"][name*="check"]').val();
		let state       = line.hasClass('bestsellers')?0:1;
		icon.addClass('loading_icon');
		$.ajax({
			type: 'POST',
			url: 'ajax/update_object.php',
			data: {'object': 'product', 'id': id, 'values': {'bestsellers': state}, 'session_id': '<?php echo $_SESSION['id'];?>
'},
			success: function(data){
				icon.removeClass('loading_icon');
				if(state)
					line.addClass('bestsellers');
				else
					line.removeClass('bestsellers');
			},
			dataType: 'json'
		});
		return false;
