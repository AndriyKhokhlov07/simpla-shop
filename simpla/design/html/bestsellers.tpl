...
{if $bestsellers}
    <div id="main_list" class="bestsellers">
        ...
        <form id="list_form" method="post">
            ...
            <div id="list">
                {foreach $bestsellers as $bestseller}
                    <div class="{if $bestseller->in_filter}in_filter{/if} row">
                        <input type="hidden" name="positions[{$bestseller->id}]" value="{$bestseller->position}">
                        <div class="move cell"><div class="move_zone"></div></div>
                        <div class="checkbox cell">
                            <input type="checkbox" name="check[]" value="{$bestseller->id}" />
                        </div>
                        <div class="cell">
                            <a href="{url module=FeatureAdmin id=$bestseller->id return=$smarty.server.REQUEST_URI}">{$bestseller->name|escape}</a>
                        </div>
        ...
        {if $categories}
            <ul>
                ...
                {foreach $categories as $c}
                    <li {if $category->id == $c->id}class="selected"{/if}><a href="index.php?module=BestsellersAdmin&category_id={$c->id}">{$c->name}</a></li>
...
<script>
    $(function() {
        ...
        $("a.in_filter").click(function() {
            ...
            let state       = line.hasClass('in_filter')?0:1;
            icon.addClass('loading_icon');
            $.ajax({
                type: 'POST',
                url: 'ajax/update_object.php',
                data: {'object': 'bestseller', 'id': id, 'values': {'in_filter': state}, 'session_id': '{/literal}{$smarty.session.id}{literal}'},
