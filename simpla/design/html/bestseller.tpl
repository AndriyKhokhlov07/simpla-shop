...
{if $bestseller->id}
    {$meta_title = $bestseller->name|escape scope=parent}
{else}
...
<form method=post id=product>
    ...
    <div id="name">
        <input class="name" name=name type="text" value="{$bestseller->name|escape}"/>
        <input name=id type="hidden" value="{$bestseller->id|escape}"/>
    </div>
    ...
    <div id="column_left">
        ...
        <div class="block">
            ...
            <select class=multiple_categories multiple name="bestseller_categories[]">
                {function name=category_select selected_id=$product_category level=0}
                    {foreach $categories as $category}
                        <option value='{$category->id}' {if in_array($category->id, $bestseller_categories)}selected{/if} category_name='{$category->single_name}'>{section name=sp loop=$level}&nbsp;&nbsp;&nbsp;&nbsp;{/section}{$category->name}</option>
    ...
    <div id="column_right">
        ...
        <div class="block">
            ...
            <ul>
                <li><input type=checkbox name=in_filter id=in_filter {if $bestseller->in_filter}checked{/if} value="1"> <label for=in_filter>Использовать в фильтре</label></li>
            </ul>
        </div>
