...
<div id="main_list">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		...
		<div class="{if !$product->visible}invisible{/if} {if $product->featured}featured{/if} {if $product->bestsellers}bestsellers{/if}  row">
...
<div id="right_menu">
	...
	<ul>
		...
		<li {if $filter=='bestsellers'}class="selected"{/if}><a href="{url keyword=null brand_id=null category_id=null page=null filter='bestsellers'}">Бестселлеры</a></li>
...
$(function() {
	...
	$("a.bestsellers").click(function() {
		...
		$.ajax({
			type: 'POST',
			url: 'ajax/update_object.php',
			data: {'object': 'product', 'id': id, 'values': {'bestsellers': state}, 'session_id': '{/literal}{$smarty.session.id}{literal}'},
			success: function(data){
				icon.removeClass('loading_icon');
				if(state)
