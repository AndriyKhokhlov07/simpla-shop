...
class ProductsAdmin extends Simpla
{
	...
	{		
		...
		if($f = $this->request->get('filter', 'string'))
		{
			if($f == 'featured')
				$filter['featured'] = 1;
            elseif($f == 'bestsellers')
                $filter['bestsellers'] = 1;
			elseif($f == 'discounted')
				$filter['discounted'] = 1; 
			elseif($f == 'visible')
				$filter['visible'] = 1; 
		...
		if($this->request->method('post'))
		{
			...
			switch($this->request->post('action'))
			{
                ...
                {
                    $this->products->update_product($ids, array('bestsellers'=>1));
                    break;
                }
                case 'unset_bestsellers':
                {
                    $this->products->update_product($ids, array('bestsellers'=>0));
                    break;
                }
			    case 'delete':
			    {
