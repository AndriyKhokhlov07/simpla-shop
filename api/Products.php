...
class Products extends Simpla
{
	...
	*/
	...
	{		
        ...
        if(isset($filter['bestsellers']))
            $bestsellers_filter = $this->db->placehold('AND p.bestsellers=?', intval($filter['bestsellers']));
        ...
        if(!empty($filter['echo_query'])) {
              echo $this->db->placehold($query); exit;
        }
