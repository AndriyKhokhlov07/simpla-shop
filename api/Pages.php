...
class Pages extends Simpla
{
	...
	*/
	public function get_page($id)
	{
		...
		$query = "SELECT id, url, header, name, meta_title, meta_description, meta_keywords, body, menu_id, position, visible, image
	...
	*/
    ...
    {
        $query = $this->db->placehold("SELECT image FROM __pages WHERE id=?", intval($page_id));
        $this->db->query($query);
        $filename = $this->db->result('image');
        if(!empty($filename))
        {
            $query = $this->db->placehold("UPDATE __pages SET image=NULL WHERE id=?", $page_id);
            $this->db->query($query);
