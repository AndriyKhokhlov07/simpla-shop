...
{* Товары Бестселлеры *}
{get_bestsellers_products var=bestsellers_products}
{if $bestsellers_products}
	<!-- Список товаров-->
	<h1>Бестселлеры</h1>
	<ul class="tiny_products">

		{foreach $bestsellers_products as $product}
			<!-- Товар-->
			<li class="product">
					...
					<form class="variants" action="/cart">
						<table>
							{foreach $product->variants as $v}
								<tr class="variant">
									<td>
										<input id="bestsellers_{$v->id}" name="variant" value="{$v->id}" type="radio" class="variant_radiobutton" {if $v@first}checked{/if} {if $product->variants|count<2}style="display:none;"{/if}/>
									</td>
									<td>
										{if $v->name}<label class="variant_name" for="bestsellers_{$v->id}">{$v->name}</label>{/if}
									</td>
									<td>
										{if $v->compare_price > 0}<span class="compare_price">{$v->compare_price|convert}</span>{/if}
